/**
 * Tests for the tools' headless-fallback branch (DGC-82): with ZERO clients
 * connected, `get_snapshot` and `export_diagram` call
 * `SnapshotOps.ensureClient` instead of erroring — and proceed down the
 * normal broker path once it reports ok.
 *
 * Driven like snapshot-tool.test.ts / export-tool.test.ts: real JSON-RPC
 * over `/mcp` on an ephemeral port. The `ensureClient` fake plays the
 * headless renderer's role by CONNECTING a real `ws` canvas client mid-call
 * — exactly what the puppeteer session does in production (the real
 * browser is exercised by the out-of-process E2E run, not here).
 */
...
function mktemp(): string {
  const dir = mkdtempSync(path.join(os.tmpdir(), "dgc-headless-"));
  tempDirs.add(dir);
  return dir;
}
...
describe("get_snapshot headless fallback", () => {
  it("renders through the client ensureClient brings up when none is connected", async () => {
    let port = 0;
    const ensureClient = vi.fn(async (target: string) => {
      await connectCanvas(port, target); // the "headless Chrome page" appears
  ...
  it("surfaces the fallback's own error verbatim (e.g. no Chrome installed)", async () => {
  ...
  it("keeps the old 'open the canvas' error when no fallback is wired", async () => {
    const { port } = await startServer({});
  ...
  it("does NOT invoke the fallback when a client is already connected (fast path)", async () => {
    let port = 0;
...
describe("export_diagram headless fallback", () => {
  it("writes the PNG rendered by the client ensureClient brings up", async () => {
    ...
    expect(readFileSync(expected)).toEqual(Buffer.from(TINY_PNG_BASE64, "base64"));
    expect(ensureClient).toHaveBeenCalledWith("demo");
  });

  it("surfaces the fallback's own error verbatim", async () => {
    const { port } = await startServer({
      exportDir: mktemp(),
      ensureClient: async () => ({
        ok: false as const,
        error: 'Call open_diagram { "name": "demo" } first, then try again.',
      }),
    });
